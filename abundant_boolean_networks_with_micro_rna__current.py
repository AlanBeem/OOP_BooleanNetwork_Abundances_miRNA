...
def plot_abundances_and_micro_rna(abundances: list[list[float]], micro_rna_abundances: list[list[list[float]]],
                                  plot_title: str, abundances_alpha: float, micro_rna_alpha: float, colors):
    ...
    plt.xticks([pa_ticks * (len(abundances) / 10) for pa_ticks in range(int(len(abundances) / 10))])
    plt.xlabel("steps")
    ...
    for pa_t in range(len(micro_rna_abundances[0])):
        ...
        for pa_s in range(len(micro_rna_abundances)):
            ...
            if max(plot_pri[-1], plot_pre[-1], plot_mirmir[-1], plot_ago_loaded[-1]) > plotted_maximum:
                plotted_maximum = max(plot_pri[-1], plot_pre[-1], plot_mirmir[-1], plot_ago_loaded[-1])
        ...
        plot_mirmir_xy = (0.618*len(abundances), plot_mirmir[int(0.618*len(abundances))])
        plot_ago_loaded_xy = (0.618*len(abundances), plot_ago_loaded[int(0.618*len(abundances))])
        plt.plot(plot_pri, linestyle='dashed', label='inline', alpha=micro_rna_alpha, color='k')
        ...
        pa_plotting_index += 1
    plt.annotate("Primary micro-RNA", xy=plot_pri_xy, xytext=(plot_pri_xy[0], plot_pri_xy[1] + 5))
    plt.annotate("Precursor micro-RNA", xy=plot_pre_xy, xytext=(plot_pre_xy[0], plot_pre_xy[1] + 5))
    plt.annotate("miRNA:miRNA*", xy=plot_mirmir_xy, xytext=(plot_mirmir_xy[0], plot_mirmir_xy[1] + 5))
    plt.annotate("Ago2-loaded miRNA", xy=plot_ago_loaded_xy, xytext=(plot_ago_loaded_xy[0], plot_ago_loaded_xy[1] + 5))
...
class MicroRNAAbundantBooleanNetwork(AbundantBooleanNetwork):
    ...
    def __init__(self, num_nodes: int, boolean_function_indices: list[int], maximum_number_of_inputs: int,
                 iterations_limit: int, setup_add_cycles: int):
        super().__init__(num_nodes, boolean_function_indices, maximum_number_of_inputs, iterations_limit,
                         setup_add_cycles)
        self.current_micro_rna_abundances_list = None
        self.additional_abundant_nodes = []  # these are all assigned True or some step calculated function
        ...
        for all_seq_i in range(1, 10):
            self.all_seq_lists.append(get_all_sequences(all_seq_i, ["A", "U", "G", "C"]))
    ...
    def add_additional_nodes(self, num_additional_nodes: int):
        self.additional_abundant_nodes.append(each_node for each_node in
                                              [AbundantNode() for pabn_i in
                                               range(num_additional_nodes)])
        self.target_sites_set_up = False
    ...
    def add_nested_nodes(self, number_of_nested_nodes: int, nodes_per_node: int, random_if_true: bool,
        ...
        super().add_nested_nodes(number_of_nested_nodes, nodes_per_node, random_if_true, num_to_average_per_update)
        self.target_sites_set_up = False

    def add_graded_nodes(self, list_of_inputs_assignments: list[list[int]]):  # these 08-26-2024
    ...
    def from_abundant_network(self, abundant_boolean_network: AbundantBooleanNetwork):
        ...
        self.abundant_n_assignments = abundant_boolean_network.abundant_n_assignments  # list of int
        self.nested_abundant_n_assignments = abundant_boolean_network.nested_abundant_n_assignments  # list[int]
        self.micro_rna_nodes = []
        self.target_sites_set_up = False
    ...
    def setup_target_site_lists(self):
        ...
        for i in range(len(all_nodes)):
            ...
            all_nodes[i].target_sites = get_three_prime_utr_target_sites(self.micro_rna_nodes, all_nodes[i])
        
        for i in range(len(all_nodes)):
            for each_site in all_nodes[i].target_sites:
                self.micro_rna_nodes[
                    self.micro_rna_nodes.index(each_site.micro_rna_node_reference)].target_sites.append(each_site)
    ...
    def knock_rates(self):
        ...
        for each_node in self.get_plottable_nodes():
            for each_site in each_node.target_sites:
                ...
                each_node.current_degradation_rate *= three_prime_utr_target_site_to_scalar(each_site)

    def advance_abundant_state(self):
        self.current_abundances_list.append([])
        for aas_i in range(len(self.abundant_nodes_1)):
            self.abundant_nodes_1[aas_i].update(self.current_states_list[-1][self.abundant_n_assignments[aas_i]])
            self.current_abundances_list[-1].append(self.abundant_nodes_1[aas_i].current_abundance)
        ...
        for aas_k in range(len(self.abundant_nodes_2)):
            self.current_abundances_list[-1].append(self.abundant_nodes_2[aas_k].current_abundance)
    ...
    def advance_mirna_abundances(self):
        self.current_micro_rna_abundances_list.append([[0, 0, 0, 0] for aka_v in range(len(self.micro_rna_nodes))])
        for aka_c, each_micro_rna_node in zip(range(len(self.micro_rna_nodes)), self.micro_rna_nodes):
            self.micro_rna_nodes[aka_c].update(True)
            self.current_micro_rna_abundances_list[-1][aka_c][0] = each_micro_rna_node.current_abundance
            self.current_micro_rna_abundances_list[-1][aka_c][1] = each_micro_rna_node.current_pre
            self.current_micro_rna_abundances_list[-1][aka_c][2] = each_micro_rna_node.current_mirmir
            self.current_micro_rna_abundances_list[-1][aka_c][3] = each_micro_rna_node.current_ago2_loaded

    def animate_boolean_states_list(self, states_to_animate: list[list[bool]], number_of_cycles: int,
        ...
        for step_i in range(num_steps):
            self.current_states_list.append(states_to_animate[step_i % len(states_to_animate)])
