...
class _BooleanFunction_K:  # index: 19, k: 0
    ...
    def __init__(self, bn, input_indices: list[int], output_function: list[bool] =[]):
        if input_indices == []:
            ...
            output_function = list(SystemRandom().choices([True, False]), k=len(input_indices))
        else:
    ...
    def get_boolean(self, a, b):
        return self.state_t[''.join([str(int(self.bn.current_states_list[-1][i])) for i in self.input_indices])]
...
functions = [_BooleanFunction1(),
             ...
             _BooleanFunction_K]  # len = 20

# Each BooleanFunctionNode gets 1 of the _BooleanFunction s
class BooleanFunctionNode:
    def __init__(self, boolean_expression_index: int):  # sort of an observation of the Factory Design pattern, but only in form, and kind of atomically
    ...
    def get_boolean(self, input_a: bool, input_b: bool):
        return self.function.get_boolean(input_a, input_b)  # TODO replace with __bool__ method
...
class CycleRecord:
    ...
    def __lt__(self, other):
        return len(self) < len(other) or self.num_observations < other.num_observations  # <
...
class CollapsedCycles(Subject):
    ...
    def add_observation(self, observed_cycle: list[list[bool]]):
        ...
        if (len(observed_cycle) == 1) or (observed_cycle[0] != observed_cycle[-1]):
            ...
            for each_state_observed in observed_cycle[0:len(observed_cycle):1]:
                self.hashed_states.update({''.join([str(int(b)) for b in each_state_observed]):self.cycle_records[-1]})
    ...
    def sort_cycle_records_by_num_observations(self):
        self.cycle_records.sort(key=lambda p: 0 - p.num_observations)  # reverse=True keyword is also an option
    ...
    def sort_cycle_records_by_hamming_diameter(self):
        self.cycle_records.sort(key=lambda p: 0 - get_hamming_diameter(p.cycle_states_list))
        self.notify()
        ...
        for each_record in self.cycle_records:
            if isinstance(each_record, CycleRecord):
                each_record.set_zero_distance(
                    per_node_sequence_similarity(zero_cycle.cycle_states_list, each_record.cycle_states_list, None))
    ...
    def get_nearest_reference(self, point: list[bool]):
        ...
        if len(self) > 0:
            for each_cycle_record in self.cycle_records:
                for each_state in each_cycle_record.cycle_states_list[0:len(each_cycle_record)]:
                    if get_hamming_distance(each_state, point) < gnr_nearest_hamm:
                        gnr_nearest_ref = self.cycle_records[self.cycle_records.index(each_cycle_record)]
            ...
            for each_cycle_record in self.cycle_records:
                for each_state in each_cycle_record.cycle_states_list[0:len(each_cycle_record)]:
                    if get_hamming_distance(each_state, point) > gfr_furthest_hamm:
                        gfr_furthest_ref = self.cycle_records[self.cycle_records.index(each_cycle_record)]
    ...
    def get_consensus_state(self):
        ...
        for gcs_i in range(len(self)):
            for gcs_j in range(len(self.cycle_records[gcs_i])):
                gcs_all_states.append(self.cycle_records[gcs_i].cycle_states_list[
                                          gcs_j])
        return get_consensus_sequence_list(gcs_all_states, [False, True])


class TrajectoryRecord:
    def __init__(self, start_index: int|None, end_index: int|None, run_in: list[list[bool]]):
...
class TrajectoriesManager(Observer):
    ...
    def __init__(self, collapsed_cycles: CollapsedCycles):
        ...
        collapsed_cycles.attach(self)  # This step is required by the Observer Subject Design Pattern
        self.u_records = []
        self.t_records = []
    ...
    def update(self):
        ...
        for each_t_trajectory_record in self.t_records:
            each_t_trajectory_record.end_index = self.collapsed_cycles.get_index(each_t_trajectory_record.run_in[-1])
        for each_u_trajectory_record in self.u_records:
            tm_update_temp_start_index = self.collapsed_cycles.get_index(each_u_trajectory_record.run_in[0])
            tm_update_temp_end_index = self.collapsed_cycles.get_index(each_u_trajectory_record.run_in[-1])
            if tm_update_temp_end_index is not None:
                self.collapsed_cycles.cycle_records[tm_update_temp_end_index].num_observations += 1
                self.add_record(TrajectoryRecord(tm_update_temp_start_index, tm_update_temp_end_index,
                                                 each_u_trajectory_record.run_in.copy()))
                self.u_records.remove(each_u_trajectory_record)


class UnitPerturbationRecord:
    def __init__(self, reference_state: list[bool], start_index: int|None, end_index: int|None,
                 run_in: list[list[bool]], perturbed_node_index: int):
    ...
    def display(self, boolean_network) -> str:
        ...
        test_bn_string_list = [str(f'{"node " + str(tbn_k + 1):>12s}: ') + str(f'{str(self.reference_state[tbn_k]):^12s}-') for tbn_k in range(len(self.run_in[0]))]  # one for each node
        ...
        print("\ncycle index: " + "".join([str(f'{str(boolean_network.bn_collapsed_cycles.get_index(self.reference_state)):^12s}')] + [str(f'{str(boolean_network.bn_collapsed_cycles.get_index(each_state)):^12s}') for each_state in self.run_in]) + "\n")
...
class CompositePerturbationRecord:
    def __init__(self, reference_state: list[bool], start_index: int|None, end_index: int|None,
                 run_in: list[list[bool]], perturbed_node_indices: list[int]):
        self.reference_state = reference_state
        self.start_index = start_index
        self.end_index = end_index
        self.run_in = run_in
...
class BooleanNetwork:
    def __init__(self, num_nodes, possible_functions_indices_list =list(range(16)),
                 maximum_inputs_number:int =2, iterations_limit: int =400, initial_sample_size: int =0):
        ...
        self.nodes = []
        self.node_inputs_assignments = []  # This class controls execution of any behaviors that are f(inputs)
        ...
        for bn_i in range(num_nodes):  # Assumes K is bounded
            init_random_ints_temp = [SystemRandom().randrange(0, num_nodes, 1)
                                        for bn_j in range(maximum_inputs_number)]
            self.node_inputs_assignments.append(init_random_ints_temp)  # The nodes don't know
        #
        # setup nodes
        for bn_k in range(num_nodes):
            self.nodes.append(BooleanFunctionNode(
                possible_functions_indices_list[SystemRandom().randrange(0, len(possible_functions_indices_list),
                                                                         1)]))
        #
        # initial sample
    ...
    def __str__(self):
        return ("BooleanNetwork: N=" + str(len(self)) + ". avg_k= " + str(self.get_avg_k()) + ". Run-ins: " +
                str(len(self.list_of_all_run_ins)) + ". Observed cycles: " + str(len(self.bn_collapsed_cycles)) +
                ". t_records: " + str(len(self.bn_trajectories.t_records)) + ". u_records: " +
                str(len(self.bn_trajectories.u_records)) + "\n" +
                "Cycle lengths: " + str([len(self.bn_collapsed_cycles.cycle_records[str_i]) for str_i in range(
                    len(self.bn_collapsed_cycles))]))  # + bn_output_string_transitions_matrix)
    ...
    def new_boolean_functions(self):
        ...
        for bn_i in range(self.num_nodes):
            self.node_inputs_assignments.append([SystemRandom().randrange(0, self.num_nodes, 1)
                                                 for bn_j in range(10)])  # The nodes don't know
        for bn_k in range(self.num_nodes):
            self.nodes.append(BooleanFunctionNode(
                self.possible_functions_indices_list[
                    SystemRandom().randrange(0, len(self.possible_functions_indices_list),
                                             1)]))
    ...
    def get_avg_k(self):
        return sum([each_node.k if self.node_inputs_assignments[gak_i][0] != self.node_inputs_assignments[gak_i][1]
                    else min(each_node.k, 1) for gak_i, each_node in
                    zip(range(len(self.nodes)), self.nodes)]) / len(self)

    def sort_cycle_records_by_num_observations(self):
        if len(self.bn_collapsed_cycles) > 1:
            self.bn_collapsed_cycles.sort_cycle_records_by_num_observations()

    def systematic_sample(self, initial_condition_list: list[list[bool]] =[[]]) -> None:
    ...
    def get_random_conditions(self):
        ...
        return [SystemRandom().choice([True, False]) for gric_i in range(len(self.nodes))]
    ...
    def run_in_from_random(self, dont_resample_if_true: bool =False):
        ...
        if len(self.set_of_initial_conditions) < 2**len(self):
            if dont_resample_if_true:
                while "".join([str(rifr_conditions[c_i]) for c_i in range(len(self))]) in self.set_of_initial_conditions:
                    rifr_conditions = self.get_random_conditions()
    ...
    def run_in_from_conditions(self, conditions: list[bool]):
        ...
        for _ in range(self.iterations_limit):
            if not (state_str:=''.join([str(int(s)) for s in self.current_states_list[-1]])) in run_d:
                ...
                self.list_of_all_indices_of_first_cycle_state.append(self.bn_collapsed_cycles.most_recent_cycle_index)
                break
        else:
    ...
    def unrecorded_states_with_noise(self, number_of_steps: int, initial_condition: list[bool],
                                     unit_perturb_noise: float =0, unit_perturb_bv_noise: float =0):
        ...
        for uswn_i in range(
                number_of_steps - 1):
            if SystemRandom().random() < unit_perturb_bv_noise * get_boolean_velocity_list_per_node(
    ...
    def run_async(self, num_steps: int, update_prob: float) -> None:
        arrays = [bytearray([SystemRandom().choice([True, False])]) for _ in self.nodes]  # initial condition
        for _ in range(num_steps):
            ...
            for i in update_index_list:  # for each node
                if SystemRandom().random() <= update_prob:
                    ...
                    arrays[i].append(self.nodes[i].get_boolean(arrays[a][-1], arrays[b][-1]))
                else:
                    arrays[i].append(arrays[i][-1])
            # all arrays in arrays are the same length
        self.current_states_list = [[arrays[i][j] for i in range(len(arrays))] for j in range(len(arrays[0]))]
    ...
    def get_avg_cycle_length(self):
        if len(self.bn_collapsed_cycles) > 0:
            return sum([len(self.bn_collapsed_cycles.cycle_records[gacl_i]) for gacl_i in
                        range(len(self.bn_collapsed_cycles))]) / len(self.bn_collapsed_cycles)
    ...
    def compute_unit_perturbations_matrix(self, sort_selection: int =1, compute_over_t_u: bool=False):
        ...
        self.total_unit_perturbations_transition_matrix = [[0 for cupm_k in range(len(self.bn_collapsed_cycles) + 1)]
                                                           for cupm_l in range(len(self.bn_collapsed_cycles) + 1)]
        self.cycles_unit_perturbations_transition_matrix = [[0 for cupm_v in range(len(self.bn_collapsed_cycles) + 1)]
        ...
        if len(self.bn_collapsed_cycles) > 0:
            for c_i in range(len(self.bn_collapsed_cycles)):  # each cycle record
                for each_state_c in self.bn_collapsed_cycles.cycle_records[c_i][0:len(self.bn_collapsed_cycles.cycle_records[c_i])]:  # each cycle starts / ends with same state
                    ...
                    for c_j in range(len(self)):
                        ...
                        c_temp_index = self.bn_collapsed_cycles.get_index(self.current_states_list[-1])
                        if (c_temp_index is not None) or (len(self.current_states_list) > self.iterations_limit):
                        ...
                        self.cycles_unit_perturbations_records.append(
                            UnitPerturbationRecord(each_state_c, c_i, c_temp_index, self.current_states_list.copy(),
                                                   c_j))
        ...
        if len(self.bn_trajectories.t_records) > 0 and compute_over_t_u:
            t_dict_list = [dict() for _ in self.bn_collapsed_cycles.cycle_records]  # index by each_t_trajectory_record.end_index
            for each_t_trajectory_record in self.bn_trajectories.t_records:
                for each_state_t in each_t_trajectory_record.run_in:
                    t_dict_list[each_t_trajectory_record.end_index].update({''.join([str(int(s)) for s in each_state_t]):(each_t_trajectory_record.end_index, each_state_t.copy())})  # try without copy, also
            ...
            for i in range(len(self.bn_collapsed_cycles.cycle_records)):
                ...
                for each_tuple in t_dict_list[i].values():
                    if self.bn_collapsed_cycles.get_index(each_tuple[1]) is None:  # excludes cycle states, data for which is present as a copy
                        ...
                        for t_j in range(len(self)):
                            ...
                            t_temp_index = self.bn_collapsed_cycles.get_index(self.current_states_list[-1])
                            ...
                            self.t_trajectories_unit_perturbations_records.append(
                                UnitPerturbationRecord(each_state_t, t_ref_index, t_temp_index,
                                                       self.current_states_list.copy(), t_j))
        #
        ...
        if len(self.bn_trajectories.u_records) > 0 and compute_over_t_u:
            ...
            for i in range(len(self.bn_trajectories.u_records)):
                ...
                for each_state_u in self.bn_trajectories.u_records[i].run_in:
                    u_dict.update({''.join([str(int(s)) for s in each_state_u]):each_state_u.copy()})
            ...
            for each_state_u in u_dict.values():
                ...
                for u_j in range(len(self)):
                    ...
                    u_temp_index = self.bn_collapsed_cycles.get_index(self.current_states_list[-1])
                    ...
                    self.u_trajectories_unit_perturbations_records.append(
                        UnitPerturbationRecord(each_state_u, u_ref_index, u_temp_index,
                                                self.current_states_list.copy(), u_j))
                
        # count them up, if present
        if len(self.cycles_unit_perturbations_records) + len(self.u_trajectories_unit_perturbations_records) + len(
                self.t_trajectories_unit_perturbations_records) > 0:
            ...
            for each_cycle_perturbation in self.cycles_unit_perturbations_records:
                ...
                #
                self.cycles_unit_perturbations_transition_matrix[cupm_temp_start_index][cupm_temp_end_index] += 1  # ADDS VALUE TO cycles_unit_...
                #
            ...
            for cupm_q in range(len(self.cycles_unit_perturbations_transition_matrix)):
                for cupm_r in range(len(self.cycles_unit_perturbations_transition_matrix[0])):
                    self.total_unit_perturbations_transition_matrix[cupm_q][cupm_r] += \
                    self.cycles_unit_perturbations_transition_matrix[cupm_q][cupm_r]
            for each_perturbation_record in (self.u_trajectories_unit_perturbations_records +
                                             self.t_trajectories_unit_perturbations_records):
                ...
                if cupm_temp_end_index is None:
                    cupm_temp_end_index = cupm_none_index
                self.total_unit_perturbations_transition_matrix[cupm_temp_start_index][cupm_temp_end_index] += 1
    ...
    def compute_composite_perturbations_matrix(self, sort_selection: int|None,
        ...
        self.total_composite_perturbations_transition_matrix = [
            ...
            for cupm_l in range(len(self.bn_collapsed_cycles) + 1)]
        self.cycles_composite_perturbations_matrix = [[0 for cupm_v in range(len(self.bn_collapsed_cycles) + 1)]
                                                      for cupm_u in range(len(self.bn_collapsed_cycles) + 1)]
        ...
        self.cycle_states_int_set = set()  # Computed to compare hash of states as binary numbers for t, u
        if len(self.bn_collapsed_cycles) > 0:
            for c_i, each_cycle in zip(range(len(self.bn_collapsed_cycles)), self.bn_collapsed_cycles.cycle_records):
                for each_state_c in each_cycle[0:len(each_cycle)]:  # each cycle starts / ends with same state
                    self.cycle_states_int_set.add(bool_state_to_int(each_state_c))
                    #  reference state: each_state_c
                    for each_composite_perturbation in composite_perturbations:
                        c_perturbed_state = each_composite_perturbation.get_perturbed_state(each_state_c)
                        c_bool_cycle_incomplete = True
                        self.set_conditions(c_perturbed_state)  # sets state
                        c_temp_index = self.bn_collapsed_cycles.get_index(self.current_states_list[-1])
                        if (c_temp_index is not None) or (len(self.current_states_list) > self.iterations_limit):
                        ...
                        self.cycles_composite_perturbations_records.append(
                            CompositePerturbationRecord(each_state_c, c_i, c_temp_index,
                ...
                for each_t_trajectory_record in self.bn_trajectories.t_records:
                    for each_state_t in each_t_trajectory_record.run_in:
                        if self.bn_collapsed_cycles.get_index(each_state_t) is None:
                            ...
                            for each_composite_perturbation in composite_perturbations:
                                t_perturbed_state = each_composite_perturbation.get_perturbed_state(each_state_t)
                                t_bool_cycle_incomplete = True
                                self.set_conditions(t_perturbed_state)
                                t_temp_index = self.bn_collapsed_cycles.get_index(self.current_states_list[-1])
                                if (t_temp_index is not None) or (
                                ...
                                self.t_trajectories_composite_perturbations_records.append(
                                    CompositePerturbationRecord(each_state_t, t_ref_index, t_temp_index,
                                                                self.current_states_list.copy(),
                                                                each_composite_perturbation.perturb_node_indices))
            if len(self.bn_trajectories.u_records) > 0:
                for each_u_trajectory_record in self.bn_trajectories.u_records:
                    ...
                    for each_state_u in each_u_trajectory_record.run_in:
                        for each_composite_perturbation in composite_perturbations:
                            u_perturbed_state = each_composite_perturbation.get_perturbed_state(each_state_u)
                            u_bool_cycle_incomplete = True
                            self.set_conditions(u_perturbed_state)
                            u_temp_index = self.bn_collapsed_cycles.get_index(self.current_states_list[-1])
                            if (u_temp_index is not None) or (
                            ...
                            self.u_trajectories_composite_perturbations_records.append(
                                CompositePerturbationRecord(each_state_u, u_ref_index, u_temp_index,
                                                            self.current_states_list.copy(),
                                                            each_composite_perturbation.perturb_node_indices))
        #
        #
        #
        if len(self.cycles_composite_perturbations_records) + len(
                self.u_trajectories_composite_perturbations_records) + len(
                self.t_trajectories_composite_perturbations_records) > 0:
            cupm_none_index = len(self.bn_collapsed_cycles)
            for each_cycle_perturbation in self.cycles_composite_perturbations_records:
                ...
                if cupm_temp_end_index is None:
                    cupm_temp_end_index = cupm_none_index
                self.cycles_composite_perturbations_matrix[cupm_temp_start_index][cupm_temp_end_index] += 1
            if compute_over_u_t_records_if_true:
                for cupm_q in range(len(self.cycles_composite_perturbations_matrix)):
                    for cupm_r in range(len(self.cycles_composite_perturbations_matrix[0])):
                        self.total_composite_perturbations_transition_matrix[cupm_q][cupm_r] += \
                        self.cycles_composite_perturbations_matrix[cupm_q][cupm_r]
                for each_perturbation_record in (self.u_trajectories_composite_perturbations_records +
                                                 self.t_trajectories_composite_perturbations_records):
                    cupm_temp_start_index = each_perturbation_record.start_index
                    cupm_temp_end_index = each_perturbation_record.end_index
                    if cupm_temp_start_index is None:
                    ...
                    if cupm_temp_end_index is None:
                        cupm_temp_end_index = cupm_none_index
                    self.total_composite_perturbations_transition_matrix[cupm_temp_start_index][
