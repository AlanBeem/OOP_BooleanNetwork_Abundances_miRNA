...
class MicroRNANode(AbundantNode):
    ...
    def setup_default_1(self, num_steps_to_maximum: int, maximum_abundance: int|float, num_steps_to_5: int):
        ...
        self.ago2_loading_rate = 0.7  # TODO What are these rates, and how do they relate to sequence?
        self.base_knockdown_rate = 0.67  # see page 11 Bartel 2009 / 2013: 0.67 as a per site knockdown
        self.current_knockdown_rate = self.base_knockdown_rate

    def __str__(self):
        return ("MicroRNA node:\nsequence: " + self.sequence +
                "\nmaximum_abundance: " + str(self.maximum_abundance) +
                "\ncurrent_abundance: " + str(self.current_abundance) +
                "\ncurrent_pre: " + str(self.current_pre) +
                "\ncurrent_mirmir: " + str(self.current_mirmir) +
                "\ncurrent_ago2_loaded: " + str(self.current_ago2_loaded) +
                "\ntranscription_rate: " + str(self.transcription_rate) +
                "\nbase_deg_rate: " + str(self.base_deg_rate) +
                "\ncurrent_degradation_rate: " + str(self.current_degradation_rate) +
                "\npri_to_pre_rate: " + str(self.pri_to_pre_rate) +
                "\npre_to_mirmir_rate: " + str(self.pre_to_mirmir_rate) +
                "\nago2_loading_rate: " + str(self.ago2_loading_rate) +
                "\nbase_knockdown_rate: " + str(self.base_knockdown_rate) +
    ...
    def update(self, state: bool):  # ODE's, for Euler's method
        ...
        self.current_ago2_loaded = (self.current_ago2_loaded * self.current_degradation_rate +
                                    self.current_mirmir * self.ago2_loading_rate)
        self.current_mirmir = ((self.current_mirmir - (
                    self.current_mirmir * self.ago2_loading_rate)) * self.current_degradation_rate +
                               self.current_pre * self.pre_to_mirmir_rate)
        self.current_pre = ((self.current_pre - (
                    self.current_pre * self.pre_to_mirmir_rate)) * self.current_degradation_rate +
                            self.current_abundance * self.pri_to_pre_rate)
        self.current_abundance = ((self.current_abundance - (
                    self.current_abundance * self.pri_to_pre_rate)) * self.current_degradation_rate +
                                  int(state) * self.transcription_rate)
...
class ThreePrimeUTRTargetSite:
    ...
    def __init__(self, start_index: int, end_index: int, mir_node_ref: MicroRNANode, abundant_node_ref: AbundantNode):
        ...
        # A1
        if self.end_index == len(self.abundant_node_reference.three_prime_utr) - 1 or self.abundant_node_reference.three_prime_utr[self.end_index + 1] == 'A':
    ...
    def __str__(self):
        disp_mir = ''.join(reversed(self.micro_rna_node_reference.sequence[1:self.binding_nt + 1]))
...
def get_three_prime_utr_target_sites(mirna_nodes: list[MicroRNANode], abundant_node: AbundantNode) -> list[ThreePrimeUTRTargetSite]:
    ...
    sequences_to_match = [mirna_nodes[sm_i].sequence[1:7] for sm_i in range(len(mirna_nodes))]  # 6nt: 1 2 3 4 5 6, for all miRNA nodes
    target_site_list = []
        ...
        for mir_i, each_seq in zip(range(len(mirna_nodes)), sequences_to_match):
            ...
            if each_seq == get_rna_reverse_complement(sub_seq):
                target_site_list.append(
                    ThreePrimeUTRTargetSite(start_index, rel_end_index,
                                            mirna_nodes[mir_i], abundant_node))  # ...TargetSite construction adjusts start_index to represent larger matches
    _assign_site_distances(target_site_list)  # This modifies target sites to represent intersite distance
    return target_site_list


def _assign_site_distances(target_site_list: list[ThreePrimeUTRTargetSite|ORFTargetSite|FivePrimeUTRTargetSite]) -> None:
    ...
    if len(target_site_list) > 1:
        if len(target_site_list) == 2:
            target_site_list[0].downstream_distance = abs(target_site_list[0].end_index - \
                                                             target_site_list[
                                                                 1].start_index)
            target_site_list[-1].upstream_distance = abs(target_site_list[-2].end_index - \
                                                            target_site_list[
                                                                -1].start_index)
        else:
            target_site_list[0].downstream_distance = abs(target_site_list[0].end_index - target_site_list[
                1].start_index)  # fencepost
            #
            for i in range(1, len(target_site_list) - 1):  # assign distances to non-fence-post portion of target site list
            ...
            target_site_list[-1].upstream_distance = abs(
                target_site_list[-2].end_index - target_site_list[-1].start_index)  # fencepost


def get_orf_target_sites(mirna_nodes: list[MicroRNANode], abundant_node: AbundantNode, basepair_bound_inclusive: int):
    ...
    for orf_i in range(len(temp_3_prime_sites)):
        if temp_3_prime_sites[orf_i].end_index - temp_3_prime_sites[orf_i].start_index >= basepair_bound_inclusive:
            temp_return_orf_sites.append(temp_3_prime_sites[orf_i])
    return temp_return_orf_sites


def get_five_prime_target_sites(mirna_nodes: list[MicroRNANode], abundant_node: AbundantNode, basepair_bound_inclusive: int):
...
for laasl_i in range(1, 10):
    local_affinity_all_seqs_lists.append(get_all_sequences(laasl_i, ["A", "U", "G", "C"]))
...
def three_prime_utr_target_site_to_scalar(target_site_to_scalar: ThreePrimeUTRTargetSite):
    ...
    if this_abundance >= 1 and mirna_abundance >= 1:  # else returns f(intermediate exponent)
        # if targeted_abundance >= 1:
        #     intermediate_scalar = mirna_abundance / targeted_abundance
        if target_site_to_scalar.end_index - target_site_to_scalar.start_index + 1 > 17:  # TODO make threshold a field in miRNA, RNA
            # subtract?                                                                   # also make the cleavability a function of location
            target_site_to_scalar.abundant_node_reference.set_abundance(target_site_to_scalar.abundant_node_reference.current_abundance - 
                                                                        (1 - 
        ...
        nt_5 = target_site_to_scalar.abundant_node_reference.three_prime_utr[target_site_to_scalar.start_index - 2:target_site_to_scalar.start_index]
        intermediate_scalar *= (1 - dinuc_vectors[0].get(nt_5, 1) / 5)  # / 10
        # print(f"* {nt_5} == {intermediate_scalar}")
        nt_3 = target_site_to_scalar.abundant_node_reference.three_prime_utr[target_site_to_scalar.end_index + 1:target_site_to_scalar.end_index + 2 + 1]
        ...
        # A1 ?
        if target_site_to_scalar.end_index + 1 < len(target_site_to_scalar.abundant_node_reference.three_prime_utr) and\
        target_site_to_scalar.abundant_node_reference.three_prime_utr[target_site_to_scalar.end_index + 1] == "A":
        ...
        i = 0
        j = 0
        possible_nt_mrna = target_site_to_scalar.abundant_node_reference.three_prime_utr[max(0,
            target_site_to_scalar.start_index - (len(target_site_to_scalar.micro_rna_node_reference.sequence) + 5)):target_site_to_scalar.start_index]
        possible_nt_mrna = list(reversed(possible_nt_mrna))  #                                                 ### m.n. 1
        possible_nt_mir = target_site_to_scalar.micro_rna_node_reference.sequence[
            1 + target_site_to_scalar.end_index - target_site_to_scalar.start_index + 1:]  # maybe not all nt of sequence- but imagine it has 1 more nt
        possible_nt_mir = get_rna_reverse_complement(possible_nt_mir)  # this should be calculated in TargetSite setup TODO
        alignments = []
        for j in range(3):
            # for i in range(len(possible_nt_mrna)):  # simple gapped alignment (only gapped on mRNA) for 4-6 intervening nucleotides on miRNA
            i = 0  #         #
            j = (j + 4) - 1  # indexes                                                                  ### m.n. 2, relates to m.n. 1
            alignments.append(0)
        ...
        if target_site_to_scalar.upstream_distance is not None:
            if target_site_to_scalar.upstream_distance <= 40:  # TODO add penalty for "too close" sites
                intermediate_scalar *= 1.2
        ...
        position = ((target_site_to_scalar.end_index + target_site_to_scalar.start_index) / 2) / len(
            target_site_to_scalar.abundant_node_reference.three_prime_utr)  # normalized to 1
        mu = 1
        sig = 0.33
        scale = 1
        cent_pen = math.e**((-0.5) * ((position - mu) / sig) ** 2)  # (thought to be general occlusion of RNA further from the end of the 3'-UTR)
        # for i in range(10):
        ...
        # site accessibility
        if target_site_to_scalar.abundant_node_reference.three_prime_utr_accessibility is not None:
            intermediate_scalar *= target_site_to_scalar.abundant_node_reference.three_prime_utr_accessibility[
                int(position * len(target_site_to_scalar.abundant_node_reference.three_prime_utr))]
        ...
        ud_nt = 80
        au_up_down_nt = [target_site_to_scalar.start_index, target_site_to_scalar.end_index]  # was 40
        while target_site_to_scalar.start_index - au_up_down_nt[0] < ud_nt and au_up_down_nt[0] > 0:  # could count the A's here and get that 3' end is more effective
            au_up_down_nt[0] -= 1
        while au_up_down_nt[1] - target_site_to_scalar.start_index < ud_nt and au_up_down_nt[0] > 0:  # could count the A's here and get that 3' end is more effective
            au_up_down_nt[1] += 1
        au_seq_5 = target_site_to_scalar.abundant_node_reference.three_prime_utr[au_up_down_nt[0]:target_site_to_scalar.start_index]
        au_seq_3 = target_site_to_scalar.abundant_node_reference.three_prime_utr[target_site_to_scalar.end_index + 1:au_up_down_nt[1]]
        # Gaussian
        mu_2 = 0
        sig_2 = 0.3
        d_nt = 1 / ud_nt  # ##
        au_local_weight = [
            (1 - x_i ** 2) * (1 / (sig_2 * math.sqrt(2 * math.pi))) * math.exp(-(1 / 2) * ((x_i - mu_2) / sig_2) ** 2)
            for x_i in [d_nt * x_j for x_j in range(ud_nt)]]  # ## per nt
        #
        au_weighted_counts = [[0, 0], [0, 0]]
        for au_i in range(len(au_seq_5)):
            au_weighted_counts[0][0] += au_local_weight[au_i] * int(au_seq_5[au_i] == "A" or au_seq_5[au_i] == "U")
            au_weighted_counts[0][1] += au_local_weight[au_i]
        for au_k in range(len(au_seq_3)):
            au_weighted_counts[1][0] += au_local_weight[au_k] * int(au_seq_3[au_k] == "A" or au_seq_3[au_k] == "U")
            au_weighted_counts[1][1] += au_local_weight[au_k]
        if au_weighted_counts[0][1] > 0:
            intermediate_scalar *= (au_weighted_counts[0][1] - au_weighted_counts[0][0]) / au_weighted_counts[0][1]
        if au_weighted_counts[1][1] > 0:
            intermediate_scalar *= (au_weighted_counts[1][1] - au_weighted_counts[1][0]) / au_weighted_counts[1][1]
        #
        # add more features #
        #
    scalar = (intermediate_scalar)**(target_site_to_scalar.abundant_node_reference.current_abundance / targeted_abundance)  # TODO what really makes sense here (physics?)
...
def utr_3_target_site_list_to_scalar(target_site_list: list[ThreePrimeUTRTargetSite]):
    if target_site_list is None or len(target_site_list) == 0:
        ...
        for each_target_site in target_site_list:
            temp_utr_3_list_scalar = temp_utr_3_list_scalar * three_prime_utr_target_site_to_scalar(each_target_site)
