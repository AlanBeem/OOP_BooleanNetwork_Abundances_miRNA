...
def run_in_as_edges(input: list[list[bool]]) -> list[list[str,str]]:
    return [[state_str(input[i]), state_str(input[i + 1])] for i in range(len(input) - 1)]


def edge_weights(input: list[list[bool]]) -> list[float]:
    return [2 * get_hamming_distance(edge[0], edge[1]) / len(edge[0]) for edge in run_in_as_edges(input)]
...
class TrajectoryCycleGraph:  # consider: ...(nx.Graph)
    ...
    def set_cycle(self, cycle_states_list: list[list[bool]]):
        ...
        self.cycle_set = self.cycle_set.union(set([state_str(s) for s in cycle_states_list]))  # {str}
        self.add_all(run_in_as_edges(cycle_states_list), edge_weights(cycle_states_list))

    def add_all(self, edges: list[list[Hashable,Hashable]], weights: list[float|int] =[]):
        """edge is [head,tail]"""
        new_edges = set()
    ...
    def visualize_nx(self, reversed: bool =True, directed: bool =False, without_cycle_state_edges: bool =False):
        ...
        for edge in (self.reversed_edges if reversed else self.edges):
            if len(edge) == 3:  # and (without_cycle_state_edges or (not edge[0] in self.cycle_set and not edge[1] in self.cycle_set)):
        ...
        nx.draw(G.subgraph(list(g for g in G.nodes if g in self.cycle_set)), node_size=15, pos=nx.kamada_kawai_layout(G), node_color='b')
